...
/* Tribune SVG background pattern */
const TRIBUNE_BG = `url("data:image/svg+xml,%3Csvg width='80' height='40' viewBox='0 0 80 40' xmlns='http://www.w3.org/2000/svg'%3E%3Crect width='80' height='40' fill='none'/%3E%3Crect x='0' y='0' width='80' height='8' fill='%23ffffff04'/%3E%3Crect x='0' y='16' width='80' height='8' fill='%23ffffff04'/%3E%3Crect x='0' y='32' width='80' height='8' fill='%23ffffff04'/%3E%3C/svg%3E")`;
...
.splash-tribune {
  ...
  background:
    ...
    radial-gradient(ellipse at 20% 50%, rgba(229,57,53,0.22) 0%, transparent 55%),
    radial-gradient(ellipse at 80% 30%, rgba(255,193,7,0.08) 0%, transparent 50%),
    radial-gradient(ellipse at 50% 100%, rgba(229,57,53,0.12) 0%, transparent 50%);
}
...
.live-tag { display: flex; align-items: center; gap: 5px; font-family: var(--cond); font-size: 11px; font-weight: 800; letter-spacing: 2px; color: #ff6b6b; text-transform: uppercase; }
...
.score-card-bg {
  ...
  background:
    ...
    linear-gradient(135deg, #0d1e35 0%, #0a1525 50%, #060e1a 100%);
}
...
.sc-name { font-family: var(--cond); font-size: 14px; font-weight: 800; text-transform: uppercase; letter-spacing: 1px; color: rgba(255,255,255,0.7); }
...
.sc-score {
  ...
  color: var(--white);
  line-height: 1;
  text-shadow: 0 0 30px rgba(229,57,53,0.35);
}
.sc-time-live {
  background: var(--red); color: #fff;
  font-family: var(--cond); font-size: 12px; font-weight: 800;
  padding: 3px 12px; border-radius: 2px; margin-top: 6px;
  display: inline-block;
}
.sc-time-ft { font-family: var(--cond); font-size: 14px; font-weight: 800; color: var(--muted); margin-top: 6px; display: inline-block; }

.sc-stats { display: flex; border-top: 1px solid rgba(255,255,255,0.06); position: relative; z-index: 1; }
.sc-stat { flex: 1; text-align: center; padding: 10px 0; border-right: 1px solid rgba(255,255,255,0.06); }
.sc-stat:last-child { border-right: none; }
.sc-stat-val { font-family: var(--display); font-size: 20px; font-weight: 700; color: var(--white); }
.sc-stat-lbl { font-family: var(--cond); font-size: 9px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; color: var(--muted); margin-top: 2px; }
...
.poll-bg {
  ...
  background:
    ...
    linear-gradient(135deg, #10180a 0%, #0d1508 100%);
}
...
.poll-opt-bar {
  ...
  background: linear-gradient(90deg, rgba(229,57,53,0.25), rgba(229,57,53,0.1));
...
.msg-text { font-size: 14px; color: rgba(255,255,255,0.8); line-height: 1.45; }
...
export default function UltrasZone() {
  ...
  const send = async () => {
    ...
    await push(ref(db, `comments/${active.id}`), {
      ...
      text: input.trim(), time: active.time,
      likes: 0, ts: Date.now()
    });
    setInput("");
  };
  ...
  const winnerIdx = pollVoted !== null ? Object.keys(pollData).reduce((a,b) => pollData[a] > pollData[b] ? a : b) : null;
                  ...
                  <div className="match-card live-card" key={m.id} onClick={() => openMatch(m)}>
                    ...
                    <div className="mc-body">
                      ...
                      <div className="mc-center">
                        ...
                        <div className="mc-status-live">{m.time}</div>
                      </div>
        ...
        <div className="score-card">
          ...
          <div className="score-inner">
            ...
            <div className="sc-mid">
              ...
              <div className="sc-time-live">{active.time}</div>
            </div>
            <div className="sc-team">
          ...
          <div className="poll-inner-wrap">
            ...
            <div className="poll-opts">
              {[active.home.short, active.away.short, "Draw"].map((opt,i) => {
                ...
                const isWinner = winnerIdx !== null && String(i) === String(winnerIdx);
                return (
                  <div
                    key={i}
            ...
            {pollVoted !== null && (
              <div className="poll-total">{pollTotal.toLocaleString()} votes total</div>
            )}
          </div>
        </div>
